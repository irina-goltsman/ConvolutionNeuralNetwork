...
def train_and_test_cross_valid(n_epochs=10, n_folds=10, non_static=False, word_embedding="-word2vec"):
    ...
    data = add_idx_features(data, word_idx_map, max_l=56, k=300, filter_h=5)
    ...
    clf = CNNTextClassifier.CNNTextClassifier(word_vect, learning_rate=0.1, seed=0, L2_reg=0.1,
                                              ...
                                              word_dimension=100, n_epochs=n_epochs, non_static=non_static)

    # kf = KFold(len(data), n_folds=n_folds, shuffle=True, random_state=0)
    # results = cross_val_score(clf, data["idx_features"], data["label"], cv=kf, n_jobs=2,
...
if __name__ == "__main__":
    ...
    train_and_test_cross_valid(n_epochs=2, n_folds=10, non_static=False)
